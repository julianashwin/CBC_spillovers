...
paragraph_lda <- full_lda_gibbs

#saveRDS(paragraph_lda, file = paste0(export_dir, "overall/short_lda_k",k,".rds"))
saveRDS(full_lda_gibbs, file = paste0(export_dir, "overall/full_lda_k",k,".rds"))
...
export_filename = paste0(export_dir, "overall/full_paragraph_topics_k",k,".csv")
...
top_terms %>%
  ...
  coord_flip()
ggsave(paste0(fig_dir, "full_all_topics_k",k,".pdf"), width = 12, height = 9)
...
export_filename = paste0(export_dir, "overall/full_meeting_topics_k",k,".csv")
...
export_filename = paste0(export_dir, "overall/full_speech_topics_k",k,".csv")
...
export_filename = paste0(export_dir, "overall/full_article_topics_k",k,".csv")
...
export_filename = paste0(export_dir, "full_topics_summary_k",k,".csv")
...
# Export
write.csv(meetinglevel_qly, paste0(export_dir, "minutes_full_qly_k",k,".csv"), fileEncoding = "utf-8", row.names = FALSE)
...
# Export
write.csv(meetinglevel_means, paste0(export_dir, "minutes_full_event_k",k,".csv"), fileEncoding = "utf-8", row.names = FALSE)


ggplot(meetinglevel_qly) + theme_bw() + 
  geom_line(aes(x = as.Date(quarter), y = T19))
...
# Export
write.csv(speechlevel_qly, paste0(export_dir, "speeches_full_qly_k",k,".csv"), fileEncoding = "utf-8", row.names = FALSE)
...
# Export
write.csv(speechlevel_means, paste0(export_dir, "speeches_full_event_k",k,".csv"), fileEncoding = "utf-8", row.names = FALSE)
...
# Export
write.csv(article_qly, paste0(export_dir, "articles_full_qly_k",k,".csv"), fileEncoding = "utf-8", row.names = FALSE)
...
write.csv(article_events, paste0(export_dir, "articles_full_event_k",k,".csv"), fileEncoding = "utf-8", row.names = FALSE)
